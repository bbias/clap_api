    ...
    if request.method == 'POST':
        ...
        for key, file in request.files.items() :
            ...
            if file and allowed_file(file.filename):
                ...
                dir = os.path.join(app.config['UPLOAD_FOLDER'], os.path.dirname(filename))
                if not os.path.exists(dir):
                    os.mkdir(dir)       
        
                file.save(os.path.join(dir, os.path.basename(filename)))
                counter += 1 
       

                #audio_files.append(os.path.join(app.root_path,app.config['UPLOAD_FOLDER'], filename))
            else: 
                print("cannot safe file - " + file)
        # calc embeddings
        #embeddings = create_embeddings(audio_files)
        print (f"Num Items - {str(num_it)}")

        print("Files saved - " + str(counter))
        end_time = time.time()
        print(latency_fmt.format(end_time - start_time))
